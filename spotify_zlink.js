...
const splice = `if(!wSocket){console.log("Connecting to server...");wSocket=new WebSocket("${cfg.useSecure ? "wss" : "ws"}://${cfg.websocket.ipAddress}:${cfg.port}");wSocket.onopen=(event)=>{wSocket.onmessage=(msg)=>{msg=JSON.parse(msg.data);console.log(msg);switch(msg.req){case "playcount":if(msg.id.length!==22)return;const uri=\`hm://album/v1/album-app/album/spotify:album:\${msg.id}/desktop?\${[\`catalogue=\${encodeURIComponent(t.catalogue)}\`, \`locale=\${encodeURIComponent(t.locale)}\`].join("&")}\`;console.log(uri);replaceme.default.resolver.get(uri,(e,t)=>{console.log(e,t);if(e){return wSocket.send(JSON.stringify({uuid:msg.uuid,success:!1,data:e}))}else{return wSocket.send(JSON.stringify({uuid:msg.uuid,success:!0,data:t.getJSONBody()}))}});break;case "hermes":const _uri=msg.uri;console.log(_uri);a.default.resolver.get(_uri,(e,t)=>{console.log(e,t);if(e){return wSocket.send(JSON.stringify({uuid:msg.uuid,success:!1,data:e}))}else{return wSocket.send(JSON.stringify({uuid:msg.uuid,success:!0,data:t.getJSONBody()}))}});break}}}}` // Inject with this string
...
lineReader.on("line", (line) => {
    ...
    if (firstLine) {
        ...
        const uriLetterRegexp = /const (.)=`hm:\/\/album\/v1\/album-app\/album\/spotify:album:\${e}\/desktop/; // Used for the regexp in resolverLetterRegexp
        const uriLetter = line.match(new RegExp(uriLetterRegexp));
        if (!uriLetter[1] || uriLetter[1].length !== 1) {
            console.error("!!! No matches for uriLetter");
            return;
        }
        const resolverLetterRegexp = `(.)\\.default\\.resolver\.get\\(${uriLetter[1]}`; // Finds letter to replace "replaceme" with in the splice variable; prevents https://github.com/evilarceus/Spotify-PlayCount/issues/9. Letter might change with every Spotify update.
        let resolverLetter = line.match(new RegExp(resolverLetterRegexp));
        if (!resolverLetter[1] || resolverLetter[1].length !== 1) {
            console.error("!!! No matches for resolverLetter");
            return;
        }
        const idx = line.indexOf(query) + query.length; // Calculation of where to insert the splice variable at
        line = line.insertAt(idx, splice.replace("replaceme", resolverLetter[1])); // Insert splice at position
        lines.push(line);
